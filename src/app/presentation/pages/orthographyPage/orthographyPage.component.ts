...
export default class OrthographyPageComponent {
  @Input() messages = signal<Message[]>([
    ...
    // \`\`\`
    // **Cómo funciona:**
    // - En WebKit (Blink/AppleWebKit) usamos los pseudo-elementos \`::-webkit-scrollbar\`, \`::-webkit-scrollbar-track\` y \`::-webkit-scrollbar-thumb\`.
  ...
  @Input() abortSignal = new AbortController();
  ...
  handleMessage(bodyForm: TextMessageBoxInterface): void {
    ...
    this.openAIService
      .consultPrompt(bodyForm.prompt, this.chatId, this.abortSignal.signal)
      .subscribe({
        next: (chunk) => {
          if (chunk.chatId) {
            this.chatId = chunk.chatId;
          }
          if (chunk.textDelta) {
            ...
            this.messages.update((prev) => {
              ...
              if (last?.isGpt) {
                next[next.length - 1] = { ...last, text: last.text + chunk.textDelta! };
              }
              return next;
            });
          }
        ...
        error: (err: unknown) => {
          ...
          this.messages.update((prev) => {
            const next = [...prev];
            const last = next[next.length - 1];
            if (last?.isGpt && last.text === '') {
              next[next.length - 1] = { isGpt: true, text: `Error: ${message}` };
            } else {
              next.push({ isGpt: true, text: `Error: ${message}` });
            }
            return next;
          });
        },
        complete: () => {
          ...
          this.messages.update((prev) => {
            ...
            const last = next[next.length - 1];
            if (last?.isGpt && last.text === '') {
              next[next.length - 1] = { isGpt: true, text: '(Sin respuesta)' };
            }
            return next;
          });
        },
